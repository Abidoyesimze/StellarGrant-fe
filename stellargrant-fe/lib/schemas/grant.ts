...
export const GrantMilestoneSchema = z.object({
  milestones: z.array(
    z.object({
      ...
      description: z.string().min(20, 'Milestone description must be at least 20 characters'),
...
export const GrantReviewersSchema = z.object({
  reviewers: z.array(
    z.string().regex(/^G[A-Z0-9]{55}$/, 'Invalid Stellar address')
  ).min(1, 'At least one reviewer is required')
    .max(7, 'Maximum 7 reviewers allowed'),
})

export const CompleteGrantSchema = GrantBasicInfoSchema
  .merge(GrantBudgetSchema)
